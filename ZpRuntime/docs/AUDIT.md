...
**Чем проверено.** Прокси из шаблона не годится для проверки: он отклоняет
учётки сам, и это видно независимо от нашего кода —
...
Поэтому поднят свой SOCKS5 с обязательной авторизацией, который записывает, чем
к нему пришли. Через `HttpClient`:
