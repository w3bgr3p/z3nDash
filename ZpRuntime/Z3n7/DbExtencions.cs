// Перенесено из z3n7/Db/DbExtencions.cs. Копия дословная.
//
// Это тот слой, которым пользуются скрипты: DbGet/DbUpd/DbQ/ClmnAdd/TblAdd и
// прочее. Наши одноимённые расширения из ZennoStub.cs удалены — они ходили в
// DevDeck.Db через StubProject.Db, эталон же на каждый вызов поднимает Sql по
// строке подключения из переменной dbSource (см. DbCore.DbQ ниже).
//
// Отсюда единственное, что нужно знать вызывающей стороне: вместо
// project.Db = new Db(...) теперь заполняется project.Var("dbSource") —
// строка "Host=..." включает pgSQL, всё остальное считается путём к SQLite.
//
// Дословно перенесены и два эталонных бага, чинить их надо в z3n7:
//   DbCore.DbQ в режиме SQLite открывает Sql(sqLitePath, null), то есть путь
//   берёт из параметра, а не из dbSource — при вызове без sqLitePath получится
//   Sql(null, null).
...
namespace z3n7
{
    ...
    public static class Get
    {
        ...
        // Отступление: #if WINDOWS. SAFU.Decode собирает HWID через WMI и сам
        // объявлен под этим же условием, поэтому DbKey доступен только в
        // net10.0-windows.
#if WINDOWS
    ...
    public static class DbTable
    {
        ...
        // Отступление: #if WINDOWS — из-за SecureVar, он под тем же условием.
        // Метод приватный и внутри DbTable никем не зовётся: MigrateTable берёт
        // одноимённый из DbMigration, а тот на SecureVar не опирается.
#if WINDOWS
