...
Под задачей две панели. Логи приходят через SSE `GET /logs/stream?task_id=...`.
HTTP-панель показывает трафик задачи из `trafficLog.jsonl`: `GET /traffic?tail=N&task_id=...`,
опрос раз в 3 секунды. Трафик пишет ZpRuntime, подробности — в [[Traffic]].
Кнопки очистки трафика нет: файл ротируется сам.
