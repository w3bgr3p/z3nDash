...
Вкладка задачи с трафиком из `trafficLog.jsonl` (раньше называлась Logs):
`GET /traffic?tail=N&task_id=...`, опрос раз в 3 секунды. Пишет его ZpRuntime,
подробности — в [[Traffic]]. Кнопки очистки нет: файл ротируется сам.

Панели логов здесь больше нет. Приём логов по HTTP убран целиком, а то, что
печатает задача, видно во вкладке Output.
