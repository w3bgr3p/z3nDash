...
Настраиваются адреса логов/HTTP-трафика, порт панели и рабочие каталоги.
...
Показывает размер и количество файлов логов. Очистка выполняется через `POST /clear-all-logs`.
