...
z3n8 — локальный веб-дашборд для управления фермой аккаунтов на базе ZennoPoster. Запускается как отдельный .NET 8 процесс на Windows-машине оператора и открывается в браузере.
...
При старте z3n8 читает `appsettings.secrets.json`. Если файл отсутствует — дашборд открывается на странице [[shift + ctrl + 0.    Config]] для первоначальной настройки. Если файл есть — сразу открывается [[shift + ctrl + 1.    z3n8 (Scheduler)]].
...
ZennoPoster задачи
    ...
    ▼
z3n8 (встроенный HTTP-сервер)
    │
    ▼
Браузер (дашборд)
```

ZP-шаблоны отправляют логи и трафик на адреса `LogHost` / `TrafficHost` из конфига. z3n8 принимает их и сохраняет в БД. Команды (start, stop, и др.) z3n8 кладёт в таблицу `_commands` — ZP-шаблон опрашивает её и выполняет.
...
|[[shift + ctrl + 1.    z3n8 (Scheduler)]]|Запуск скриптов по расписанию|
...
- [[01. Настройка сервера]] — первичная настройка z3n8: БД, порты, jVars, импорт данных фермы
...
### [[shift + ctrl + 1.    z3n8 (Scheduler)]]

Запуск скриптов по расписанию.

- [[shift + ctrl + 1.    z3n8 (Scheduler)#Список задач|Список задач]] — группировка, статусы, executor-фильтр
- [[shift + ctrl + 1.    z3n8 (Scheduler)#Создание и редактирование задачи|Создание задачи]] — имя, путь, executor, args, enabled
- [[shift + ctrl + 1.    z3n8 (Scheduler)#Расписание|Расписание]] — cron, interval, fixed time, overlap policy
- [[shift + ctrl + 1.    z3n8 (Scheduler)#Execution tab|Execution tab]] — статус, PID, uptime, memory, parallel instances
- [[shift + ctrl + 1.    z3n8 (Scheduler)#Output tab|Output tab]] — вывод последнего запуска
- [[shift + ctrl + 1.    z3n8 (Scheduler)#Payload — Set Values и Build Schema|Payload]] — Set Values, Build Schema, Import, Export
- [[shift + ctrl + 1.    z3n8 (Scheduler)#Logs панель|Logs панель]] — логи задачи + SSE live
- [[shift + ctrl + 1.    z3n8 (Scheduler)#HTTP панель|HTTP панель]] — трафик задачи + SSE live
